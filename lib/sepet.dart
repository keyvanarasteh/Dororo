...
class sepet extends StatefulWidget {
  sepet({required this.s,required this.a});
  final int s;
  final int a;
  @override
  State<sepet> createState() => _sepetState(s: s , a: a);
}
...
class _sepetState extends State<sepet> {
  ...
  _sepetState({required this.s,required this.a});
  final int s ;
  final int a;
  double fiyat = 0; 
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _selectedIndex== 0 ? anasayfa() : count == 0 ? Container(child: Center(child: Text("Sepet Boş..", style: TextStyle(fontSize: 25, color: Colors.blueGrey),))) 
      :
      Container(
        child: ListView(
          children: [
            Column(
              ...
              children:[
               for(var item in sepett ) sepettt(sepett.toList().indexOf('$item'),item),
               
                ],
            ),
            Container(color: Colors.black,height: 1),  

            for(var item in sepett ) Text((toplam += Anahtar2[Anahtar1.indexOf(item)] * adet[sepett.toList().indexOf(item)]).toString(), style: TextStyle(color: Colors.blueGrey[400], fontSize: 1)),
  ...
  Widget sepettt(int i , String item ) {
    ...
    double f = ( Anahtar2[Anahtar1.toList().indexOf('$item')] * adet[i]).toDouble();
          ...
          child: Row(
            ...
            children: [
              ...
              Column(
                ...
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  InkWell(child: Icon(Icons.add_circle_outline), onTap: () {setState(() { adet[i]++ ;});}),
                  Text("Adet : ${adet.isEmpty ? '0' : adet[i]}"),
                  
                  InkWell(child: Icon(Icons.remove_circle_outline),onTap: () {setState(() {adet[i]--;});},), 
                ],
              ),
            ],
